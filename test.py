...
def Email(Modtagermail, file_name, file_path):
    ...
    <body>
        <p>Hej HR! :) </p>
        <p>Hermed som aftalt med Flemming Sommer Bach indtastningsgrundlag i forhold til SD Løn.</p> 
    </body>
    </html>
    """
...
def upload_to_sharepoint(ctx, file_path, folder_url: str, site_url_str: str, max_retries: int = 6):
    """
    Robust upload: kopierer først til %TEMP% (undgår OneDrive/AV-lås), så upload.
    - ctx: ClientContext (allerede med .with_credentials(...))
    - file_path: lokal fil (Path/str)
    - folder_url: server-relativ sti (fx '/teams/xxx/Delte dokumenter/Testmappe')
                  eller relativ ('Delte dokumenter/Testmappe') eller fuld URL.
    - site_url_str: den oprindelige site-URL streng brugt til ClientContext(...)
    """
